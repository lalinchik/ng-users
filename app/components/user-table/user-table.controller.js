  ...
  this.columnList = [
    {
      ...
      hidden: false,
      sortable: false,
      renderer: (user) => `<img class="avatar" src="${user.picture.thumbnail}">`
    },
    {
      title: 'Name',
      hidden: false,
      sortable: true,
      value: (user) => `${user.name.first} ${user.name.last}`,
      renderer: (user) => `${user.name.first} ${user.name.last}`
    },
    ...
    {title: 'Nationality', hidden: false, sortable: true, value: (user) => user.nat, renderer: (user) => user.nat},
    {
      ...
      renderer: (user) => `<a href="mailto:${user.email}">${user.email}</a>`
    }
  ];
